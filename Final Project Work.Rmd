...
SpeedDating %>% 
  ...
  scale_fill_discrete(name = "Yes or no", labels = c("No", "Yes"))
```
In the above graphs, we can see that intelligence doesn't play a significant role in male decisions to pick a female partner and mostly all the men said yes to females with all kinds of ratings.
In the case of females though we can see that intelligence can play a significant role in determining a male partner since we can see that mostly females said no to males rated below 5 in intelligence.
...
SpeedDating %>% 
  ...
  scale_fill_discrete(name = "Yes or no", labels = c("No", "Yes"))
```
For the Fun attributes we can see that both males and females mostly said no to the partner rated below five. From this, we can conclude that attributes can play a significant role in determining a partner.
...
SpeedDating %>% 
  ...
  scale_fill_discrete(name = "Yes or no", labels = c("No", "Yes"))
```
In the case of the sincere attribute, we can see from the graphs that it's an important attribute in the case of females however in the case of males it seems like it doesn't matter much as most of the males yes to females with even low ratings.
...
SpeedDating %>% 
  ...
  scale_fill_discrete(name = "Yes or no", labels = c("No", "Yes"))
```
From the above graphs, we can say that the ambitious attribute can play a significant role in determining partner as see that both males and females mostly said no to persons below the rating of five. 
...
SpeedDating %>% 
  ...
  scale_fill_discrete(name = "Yes or no", labels = c("No", "Yes"))
```
From the above graphs, we can say that the shared intersect attribute doesn't play a significant in determining the partner. 
```{r}
...
```
We can see that for both male and female attractive attribute has the highest coefficient value therefore we can say that it plays a major role in deciding the partner. However, when we plot the graph we see that no men wanted to date a girl below the rating of 5 in the case of females the rating didn’t have much of an effect on deciding on a dating partner.
